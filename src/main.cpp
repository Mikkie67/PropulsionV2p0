...
//->set_sk_server("192.168.10.3", 80)
  ...
  //     sound) zonesTemp                            // zones array of zone
  //     structs
  // );
  ...
  app.onRepeat(10, [](){if ((int)shaftHz != (int)clShaftFreq.getCurrentFrequency()) shaftHz = clShaftFreq.getCurrentFrequency();});
  sensesp_app->start();
}
void loop() {
  app.tick();
  // Serial.println(clShaftFreq.getCurrentFrequency());
  if (updateRpm) {
  ...
  //   updateRpm = false;
  // }
  // else if ((shaftHz > 1.0)&&((millis()-milliLastTime) > 100)){
  //     float decayRate = 1000000;
  //   //  Serial.println(exp(-decayRate / (micros() - lastTime)));
  //     //shaftHz = shaftHz * exp(-decayRate / (micros() -
  //     lastTime));//timeSinceLastPulse);
  //    Serial.println(shaftHz);
  //     //timeDifference = 0;
